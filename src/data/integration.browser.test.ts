...
describe("when user logged in", function ()
{
    async function helper_insert_wiki_data_component(test_title: string | undefined, override?: Partial<DataComponent>)
    {
        ...
        const data_component_to_insert: DataComponent = {
        ...
        const response = await insert_data_component(get_supabase, data_component_to_insert)
        ...
        const data_component = response.data

        return { user_id, data_component, response }
    }


    async function helper_insert_user_owned_data_component(test_title: string | undefined, override?: Partial<DataComponent>)
    {
        user_id = await check_user_is_logged_in(user_id)
    ...
    describe("wiki data components", function ()
    {
        this.timeout(5000)
        ...
        {
            const { data_component: inserted_data_component } = (await helper_insert_wiki_data_component(this.test?.title))
            const data_component = {
        ...
        it("should paginate over the test data components in the database", async function ()
        {
            ...
            const { data_component: inserted_data_component_1 } = (await helper_insert_wiki_data_component(this.test?.title))
            ...
            const { data_component: inserted_data_component_2 } = (await helper_insert_wiki_data_component(this.test?.title, { id: new IdAndVersion(-2, 1) }))
        ...
        {
            await helper_insert_wiki_data_component(this.test?.title, { title: "abc", description: "some description" })
    ...
    describe("user owned data components", function ()
    {
        this.timeout(5000)

        const data_component_fixture: DataComponent = Object.freeze(init_data_component({}, true))

        let do_not_delete_test_data = false
        afterEach(async () =>
        {
            if (do_not_delete_test_data) return
            // Clean up test data after all tests have run
            await delete_test_data_in_db("data_components_history")
            await delete_test_data_in_db("data_components")
        })
        ...
        describe("searching over user owned data components", function ()
        {
            before(async () =>
            {
                do_not_delete_test_data = true
                // Clean up test data after all tests have run
                await delete_test_data_in_db("data_components_history")
                await delete_test_data_in_db("data_components")
            })
            ...
            it("set up data for tests", async function ()
            {
                inserted_wiki_data_component = (await helper_insert_wiki_data_component(this.test?.title, { title: "Wiki Component", description: "some description" })).data_component
                inserted_user_owned_data_component = (await helper_insert_user_owned_data_component(this.test?.title, { id: IdAndVersion.from_str("-2v1") })).response.data
            })
            ...
            it(`should filter by a user_id`, async function ()
            {
                ...
                deep_equals(
                    ...
                    `Expected search results to only include the "user owned" data components matching the filter_by_owner_id`
                )
            })
        })
    })
